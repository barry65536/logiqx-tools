...
#define DATLIB_VERSION "v2.36bh"
#define DATLIB_DATE "22 February 2012"
	...
	for (i=0, curr_game=dat->games; i<dat->num_games; i++, curr_game++)
	{
		for (j=0, curr_sample=curr_game->samples; j<curr_game->num_samples; j++, curr_sample++)
		{
			...
			while (game_sampleof)
			{
				...
				if (game_sampleof->game_sampleof)
				{
					if (!strcmp(game_sampleof->game_sampleof->name, game_sampleof->name))
					{
						/* --- Starting with MAME 0.145u1, all games using samples have a sampleof reference, --- */
						/* --- Here we check to see if the current set has the same name and sampleof reference, --- */
						/* --- and that signifies we have reached the sample parent --- */
						break;
					}
				}
